...
for (x in clusterNames) {
  ...
  meigUpdated <- as.matrix(sapply(1:nrow(coords),
                           function(i) maxEigenValReplacingCor(pairCount, W[i,])))
  ...
  pmeigUpdated <- matrix(nrow = nitr, ncol = nrow(coords))
  pmeigUpdated <- sapply(1:nitr, function(i) {
    x <- pairCount
    o = sample(1:nrow(coords))
    x <- t(sapply(1:nrow(pairCount), function(j) {
      x[j,] = pairCount[j,o]
    ...
    pmeigUpdated[i,] = sapply(1:nrow(W), function(j) maxEigenValReplacingCor(x, W[j, ]))
  })
  ...
  pvals_eigUpdated <- matrix(nrow = nrow(coords), ncol = 1)
  pvals_eigUpdated <- as.matrix(sapply(1:nrow(meig), function(i) {
    pvals_eigUpdated[i,] = sum(pmeigUpdated[i,] > meigUpdated[i]) / nitr
  }))
  
  
  df_res <- data.frame(x = coords[,"x"],
                       y = coords[,"y"],
                       meigUpdated = meigUpdated,
                       pvals_eigUpdated = pvals_eigUpdated
                       )
  
  pdf(paste0("output/pval_plots_cancer/", x, ".pdf"),
      height = 6, width = 10, onefile = TRUE)
  plotdf(df_res,df_res$meigUpdated,df_res$pvals_eigUpdated,"Weighted Correlation", "-log10(pval)")
  dev.off()
