...
coordinateIndex= c(203,397,400,570,640)
cellCoord <-c()
for ( i in coordinateIndex)
{
  cellCoord<- c(cellCoord,paste0(coords[i,1],'x',coords[i,2]))
}
...
for (x in clusterNames) {
  ...
  meig <- as.matrix(sapply(1:nrow(coords),
                           function(i) maxEigenVal(pairCount, W[i,],coordinateIndex,i)))
  ...
  pmeig <- sapply(1:nitr, function(i) {
    ...
    pmeig[i,] = sapply(1:nrow(W), function(j) maxEigenVal(x, W[j, ],coordinateIndex,j))
  })
