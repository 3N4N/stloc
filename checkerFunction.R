...
maxEigenVal <- function(x, w=1,coordinateIndex,iterationCount=-1) {
  ...
  if(iterationCount %in% coordinateIndex)
  {
    cat( '\n', iterationCount,'\n',file = "output/outfile.txt",append= TRUE )
    # cat('Correlation matrix ',  cor(x),file="output/outfile.txt",sep=" ")
    write.table(cor(x),file = "output/outfile.txt", row.names = FALSE, sep = "\t",append=TRUE)
    cat('max eigen value is ',max(eigen(cor(x),only.values=TRUE)$values),'\n\n',file="output/outfile.txt",sep=" ",append = TRUE)
  }
