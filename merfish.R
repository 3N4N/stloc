...
dataset$new_X <- (floor(dataset$Centroid_X /100.0) *100 ) +50
dataset$new_Y <- (floor(dataset$Centroid_Y /100.0) *100) +50
...
{
    if( sqrt((dataset[k,]$Centroid_X -dataset[k,]$new_X)^2 +(dataset[k,]$Centroid_Y - dataset[k,]$new_Y )^2 ) >=27.5)
    {
        next
    }
    print(k)
...
write.table(tempDataFrame,"./data/moffit/merfishSpatial.csv",row.names = FALSE,append= FALSE)
